...
function renderExecutiveTab() {
    ...
    if (data.executiveSummary) {
        ...
        updateKPI('kpiTotalRevenue', Utils.formatCurrency(es.total_revenue), 
            es.revenue_growth_pct, 'vs last 30 days');
        updateKPI('kpiTotalOrders', Utils.formatNumber(es.total_orders),
            es.orders_growth_pct, 'vs last 30 days');
        ...
        if (es.reference_date) {
            document.querySelector('.freshness-text').textContent = 
                `Data as of ${new Date(es.reference_date).toLocaleDateString('en-IN')}`;
        }
    }
    ...
    if (data.monthlyTrend && data.monthlyTrend.length > 0) {
        ...
        ChartManager.createLineChart('chartRevenueTrend',
    ...
    if (data.categories && data.categories.length > 0) {
        ...
        ChartManager.createDoughnutChart('chartCategoryRevenue',
    ...
    if (data.topProducts && data.topProducts.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = data.topProducts.slice(0, 10).map((p, i) => `
                <tr>
                    <td>${i + 1}</td>
                    <td class="truncate" title="${p.productName}">${Utils.truncate(p.productName, 25)}</td>
                    <td>${p.category}</td>
    ...
    if (data.topCustomers && data.topCustomers.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = data.topCustomers.slice(0, 10).map((c, i) => `
                <tr>
                    <td>${i + 1}</td>
                    <td>${c.fullName}</td>
                    <td>${c.city}</td>
                    <td class="text-right">${Utils.formatCurrency(c.totalRevenue)}</td>
                </tr>
            `).join('');
        }
    }
...
function renderSalesTab() {
    ...
    if (data.recentTrend && data.recentTrend.length > 0) {
        const sorted = [...data.recentTrend].sort((a, b) => 
            new Date(a.date) - new Date(b.date));
        
        ChartManager.createLineChart('chartDailySales',
            sorted.map(d => new Date(d.date).toLocaleDateString('en-IN', {day: '2-digit', month: 'short'})),
            [{
                label: 'Revenue',
    ...
    if (data.dayOfWeek && data.dayOfWeek.length > 0) {
        ...
        ChartManager.createBarChart('chartDayOfWeek',
            sorted.map(d => d.dayName),
            [{
                label: 'Revenue',
                data: sorted.map(d => d.revenue),
                backgroundColor: sorted.map(d => d.isWeekend ? CONFIG.chartColors.success : CONFIG.chartColors.primary)
            }]
        );
    }
    ...
    if (data.quarterly && data.quarterly.length > 0) {
        ...
        ChartManager.createBarChart('chartQuarterly',
    ...
    if (data.paymentModes && data.paymentModes.length > 0) {
        ChartManager.createDoughnutChart('chartPaymentModes',
...
function renderProductsTab() {
    ...
    if (data.topProducts && data.topProducts.length > 0) {
        ...
        ChartManager.createHorizontalBarChart('chartTopProductsBar',
    ...
    if (data.abcAnalysis && data.abcAnalysis.summary) {
        ChartManager.createDoughnutChart('chartABCPie',
            data.abcAnalysis.summary.map(s => `Class ${s.class} (${s.productCount} products)`),
    ...
    if (data.categories && data.categories.length > 0) {
        ChartManager.createBarChart('chartCategoryPerformance',
    ...
    if (data.inventoryStatus && data.inventoryStatus.length > 0) {
        ChartManager.createDoughnutChart('chartInventoryStatus',
    ...
    if (data.topProducts && data.topProducts.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = data.topProducts.slice(0, 15).map(p => `
                <tr>
                    <td>${p.revenueRank}</td>
                    <td class="truncate" title="${p.productName}">${Utils.truncate(p.productName, 30)}</td>
                    <td>${p.category}</td>
                    <td>${p.brand}</td>
                    <td class="text-right">${Utils.formatCurrency(p.revenue)}</td>
                    <td class="text-right">${Utils.formatNumber(p.unitsSold)}</td>
                    <td>${p.avgRating ? p.avgRating.toFixed(1) + ' ⭐' : '-'}</td>
                    <td class="text-right">${Utils.formatNumber(p.currentStock)}</td>
                </tr>
            `).join('');
        }
    }
...
function renderCustomersTab() {
    ...
    if (data.rfmSegments && data.rfmSegments.length > 0) {
        ChartManager.createHorizontalBarChart('chartRFMSegments',
    ...
    if (data.demographics && data.demographics.length > 0) {
        ...
        const maleData = ageGroups.map(ag => 
            data.demographics.find(d => d.ageGroup === ag && d.gender === 'M')?.customerCount || 0);
        const femaleData = ageGroups.map(ag => 
            data.demographics.find(d => d.ageGroup === ag && d.gender === 'F')?.customerCount || 0);
        
        ChartManager.createBarChart('chartDemographics',
            ageGroups,
            [{
                label: 'Male',
                data: maleData,
    ...
    if (data.churnRisk && data.churnRisk.highPriorityCustomers) {
        ...
        if (tbody) {
            tbody.innerHTML = data.churnRisk.highPriorityCustomers.slice(0, 10).map(c => `
                <tr>
                    <td>${c.fullName}</td>
                    <td>${c.clvTier}</td>
                    <td>${Utils.formatCurrency(c.totalSpent)}</td>
                    <td>${c.daysInactive} days</td>
                    <td class="truncate" title="${c.recommendedAction}">${Utils.truncate(c.recommendedAction, 40)}</td>
                </tr>
            `).join('');
        }
    }
...
function renderStoresTab() {
    ...
    if (data.regional && data.regional.length > 0) {
        ChartManager.createBarChart('chartRegionalPerformance',
    ...
    if (data.topStores && data.topStores.length > 0) {
        ...
        ChartManager.createHorizontalBarChart('chartStoreRankings',
    ...
    if (data.topStores && data.topStores.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = data.topStores.slice(0, 15).map(s => {
                const tierBadge = s.performanceTier === 'Star' ? 'success' :
                    s.performanceTier === 'Average' ? 'info' :
                    s.performanceTier === 'Improving' ? 'warning' : 'danger';
                return `
                    <tr>
                        <td>${s.revenueRank}</td>
                        ...
                        <td>${s.city}</td>
                        <td>${s.region}</td>
                        <td class="text-right">${Utils.formatCurrency(s.revenue)}</td>
                        <td class="text-right">${Utils.formatCurrency(s.profit)}</td>
                        <td class="text-right">${s.profitMargin}%</td>
                        <td>${Utils.createBadge(s.performanceTier, tierBadge)}</td>
                    </tr>
                `;
            }).join('');
        }
...
function renderOperationsTab() {
    ...
    if (data.returns && data.returns.byCategory) {
        const totalReturns = data.returns.byCategory.reduce((sum, c) => sum + (c.returnCount || 0), 0);
        const totalRefunds = data.returns.byCategory.reduce((sum, c) => sum + (c.totalRefunds || 0), 0);
    ...
    if (data.deliveryPerf && data.deliveryPerf.length > 0) {
        ...
        ChartManager.createLineChart('chartDeliverySLA',
    ...
    if (data.couriers && data.couriers.length > 0) {
        ChartManager.createBarChart('chartCourierPerformance',
    ...
    if (data.returns) {
        ...
        if (data.returns.byCategory && data.returns.byCategory.length > 0) {
            const withReturns = data.returns.byCategory.filter(c => c.returnRate > 0);
            ChartManager.createBarChart('chartReturnByCategory',
                withReturns.map(c => c.category),
                [{
                    label: 'Return Rate %',
                    data: withReturns.map(c => c.returnRate),
                    backgroundColor: withReturns.map(c => 
                        c.returnRate > 15 ? CONFIG.chartColors.danger :
                        c.returnRate > 10 ? CONFIG.chartColors.warning : CONFIG.chartColors.success
                    )
                }]
            );
        }
...
function renderMarketingTab() {
    ...
    if (data.campaigns && data.campaigns.length > 0) {
        ...
        ChartManager.createBarChart('chartCampaignROI',
            withROI.map(c => Utils.truncate(c.campaignName, 15)),
            [{
                label: 'ROI %',
                data: withROI.map(c => c.roi || 0),
                backgroundColor: withROI.map(c => 
                    (c.roi || 0) > 100 ? CONFIG.chartColors.success :
                    (c.roi || 0) > 0 ? CONFIG.chartColors.warning : CONFIG.chartColors.danger
                )
            }]
        );
    }
    ...
    if (data.channels && data.channels.length > 0) {
        ChartManager.createDoughnutChart('chartChannelPerformance',
    ...
    if (data.campaigns && data.campaigns.length > 0) {
        ...
        if (tbody) {
            tbody.innerHTML = data.campaigns.slice(0, 10).map(c => {
                const statusBadge = c.status === 'Completed' ? 'success' :
                    c.status === 'Active' ? 'info' : 'warning';
                return `
                    <tr>
                        <td class="truncate" title="${c.campaignName}">${Utils.truncate(c.campaignName, 25)}</td>
                        <td>${Utils.createBadge(c.status, statusBadge)}</td>
                        <td class="text-right">${Utils.formatCurrency(c.budget)}</td>
                        <td class="text-right">${Utils.formatCurrency(c.actualSpend)}</td>
                        <td class="text-right">${Utils.formatNumber(c.clicks)}</td>
                        <td class="text-right">${Utils.formatNumber(c.conversions)}</td>
                        <td class="text-right">${c.conversionRate || 0}%</td>
                        <td class="text-right ${(c.roi || 0) > 0 ? 'text-success' : 'text-danger'}">
                            ${c.roi || 0}%
                        </td>
                    </tr>
                `;
...
function updateAlertBadge() {
    ...
    if (badge && state.data.alerts) {
        const count = state.data.alerts.alerts ? state.data.alerts.alerts.length : 0;
...
function showAlertModal() {
    ...
    if (state.data.alerts && state.data.alerts.alerts) {
        body.innerHTML = state.data.alerts.alerts.map(a => `
